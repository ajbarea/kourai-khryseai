...
- **Frontend:** buildless [lit](https://lit.dev) Web Components (light DOM + one `styles.css`), loaded
  via an import map from **vendored, offline lit** (`web/vendor/lit/`, no CDN at runtime — regenerate
  with `scripts/vendor-lit.sh`). Components: `kk-app`, `kk-rail`, `kk-terminal`, `kk-hud`, `kk-prompt`.
...
`src/gateway.js` (NDJSON client), `src/audio.js` (TTS playback), `src/app.js` (components),
`vendor/lit/` (vendored offline lit, regenerated by `scripts/vendor-lit.sh`).

## Next

- **M5** — shipped: TTS playback, vendored offline lit, the `kourai-dev web` launcher. Still open:
  mobile/responsive pass, `@agent` targeting, and a profile-create step for first-run web users.
