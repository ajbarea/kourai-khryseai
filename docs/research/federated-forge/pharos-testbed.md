...
Pharos therefore runs a surface-only probe over every corpus version: a
classifier given only non-semantic features, report length, timestamp
regularity, sender, formatting, field presence, and no content word at all.

**The requirement is not that it score at chance, and an earlier revision of
this spec was wrong to say so.** That version demanded AUC
between 0.45 and 0.55 and treated the band as pass/fail. Building the corpus
refuted the premise: ground truth here is defined by the *presence of
particular content*, so the facts that make an event significant are
present more often in significant events by construction, and shape carries
some information about them necessarily. A corpus that scored at chance
would be one whose labels its own text does not support. Four generator
revisions were spent chasing a target that a correct corpus cannot hit.

What replaced it is a calibration instrument rather than a purity test.
The baseline must be **measured, bounded, and published**, and it must be
significant against a **permutation null** computed under the identical
procedure --- which is what makes it comparable at all, since the absolute
AUC is not comparable across corpora with different splits and class
balances. Every downstream score is then reported against that baseline
rather than against 0.5. On the current corpus the surface baseline is
roughly 0.65 against a null near 0.50.

The reframing is not special pleading for one generator. The same probe run
against three public corpora exceeds its own permutation null on every one
of them, including an adversarially filtered benchmark where the leak is
reduced but not removed. Any corpus whose labels are defined by content has
this property; what distinguishes an honest one is that it reports the
number.

The probe result ships in the manifest for every corpus version, and a
version whose baseline is unmeasured, insignificant against its own null, or
above the published ceiling cannot support a triage claim.
...
!!! note "What Pharos built first, and why it is not this"

    The requirements below aim at **realism**, and they are the right target for
    the personalization ablation: a homogeneous simulator leaves the personal
    adapter nothing to learn. Pharos's implemented `pharos.analyst` does something
    else on purpose. Its reviewers are decision procedures with named parameters
    --- escalation threshold, slip rate, revision rate, whether grounds are named,
    whether the reviewer escalates --- and no language model is involved.

    The reason is that the two goals need opposite things. Measuring whether the
    rule *survives* review needs a feedback process whose ground truth is known by
    construction, and a persona-prompted simulator puts an unquantified error
    exactly where the measurement goes: persona variables explain only a small
    fraction of annotator variance, and model judgments compress disagreement
    rather than reproducing it ([Xu and Jurgens
    2026](https://arxiv.org/abs/2601.09065)). Measuring whether personalization
    has anything to learn needs the opposite --- heterogeneity that a parameter
    grid cannot manufacture honestly.

    So both are wanted, for different experiments, and neither substitutes for the
    other. The parameter grid bounds a mechanism and estimates no population;
    everything it produces carries that cap. Evolved persona policies remain the
    requirement for the personalization arm and have not been built.
