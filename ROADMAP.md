...
- 2026-05-02 — **UX/DX wins shipped** in two PRs. `[#118]` dropped dead `color` and `hex_color` fields from `shared/src/kourai_common/agents.py:AGENT_METADATA` — three separate per-agent color dicts had silently disagreed on values; nothing actually read the canonical pair (only `hosts/gui/maidens.py:AGENTS["color"]` and `hosts/gui/agent_personality_indicators.py:DEFAULT_PERSONALITIES` are live). Tightened type annotation, updated module docstring; -19 dead lines. `[#119]` shipped per-agent Okabe-Ito CVD-safe badges in the CLI comms-window header (closes IMPL bug #10) — each maiden's name now renders as a colored-background chip, six core specialists each get a distinct Okabe-Ito hue, dokimasia yellow gets bold-black for WCAG contrast (~12:1), companions/support land on shared green or neutral gray. Honors NO_COLOR (no-color.org spec — strips bg+fg, keeps bold per spec) and existing `_has_truecolor` detection (16-color terminals fall back to bold-only rather than approximate Okabe-Ito hex into ANSI 16, which would silently drift the CVD-safety property). Web-search confirmed Okabe-Ito as the 2026 categorical-accessibility standard; March-2026 Claude Code issue #34702 surfaced exactly the "hardcoded RGB bypasses terminal theme" failure mode the truecolor + NO_COLOR gates avoid. 8 new unit tests; live preview validated all 11 agents. 2894 unit tests green; lint + format + ty clean.
