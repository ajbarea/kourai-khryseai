...
- 2026-05-23 — **Aletheia v2 PR 6: `docs/agents/aletheia.md` user-facing documentation** [#234]. Two-surface reference covering v1 generic claim validation + v2 `verify_and_cite` academic citation verification. Anti-hallucination guarantee inventory (5 mechanical + 3 probabilistic), Python / code-comment / Markdown usage examples, config table, operational notes (latency, drift cadence, override). Specialists.md links to it; zensical nav under Agents. **Closes the 6-PR Aletheia v2 arc** (#222 design spec through #234 docs). Dogfooded a `check_citations.py` improvement: the new `_strip_md_code_fences` helper drops fenced code blocks before scanning so user-facing docs can carry illustrative `# see docs/citations/example.md` examples without false-positive failures from PR 5's gate. Two new unit tests cover both directions (fenced examples ignored, Python comments still match).
- 2026-05-23 — **Aletheia v2 Task 4.6: Nightly API-contract drift tests** [#233]. Three real-API calls per night (Semantic Scholar / OpenAlex / arXiv) assert the response fields `verify_and_cite` depends on are still present. Schema breakage upstream surfaces as a red nightly run hours before it would break the cassette-replay tests on the push/PR fast lane. New `tests/nightly/test_api_contracts.py` (3 tests); new `nightly.yml` `aletheia-v2-contracts` job installs `--extra aletheia-v2`, passes `S2_API_KEY` through; the S2 test skips cleanly when the secret is unset (unauthenticated S2 rate-limits too hard for a 3-test run).
- 2026-05-23 — **Aletheia v2 PR 5: mechanical `check_citations.py` CI gate** [#232]. No-LLM filesystem walker that finds `docs/citations/<slug>.md` references in `.py` / `.md` / `.yml` / `.yaml` files and verifies each resolves to a well-formed YAML-frontmatter artifact (flags stale `verified_at` > 365 days). Wired into pre-commit (`.pre-commit-config.yaml`) and a dedicated `.github/workflows/check-citations.yml` with path-based triggering. 6 unit tests + smoke run against the kourai repo green. Independent of PR 4 — could have been parallel.
- 2026-05-23 — **Aletheia v2 PR 4: `verify_and_cite` + `audit_existing_citations`** [#230]. Five-step ReAct loop — extract_claim → search_papers → fetch_paper_text → match_evidence (verbatim substring gate) → triangulate → write artifact. `audit_existing_citations` re-runs the triangulation gate against every artifact in `docs/citations/` for scheduled drift detection. Tools (`aletheia_extract_claim`, `aletheia_search_papers`, `aletheia_fetch_paper_text`, `aletheia_match_evidence`, `verify_and_cite`, `audit_existing_citations`) append to v1 `agents/aletheia/agent.py` — function-level httpx imports + `contextlib.suppress(ImportError)` keep unit tests passing without the extra. FakeLLM fixture (scripted-response stand-in) enables deterministic agent tests without real LLM calls. CI workflow extended to run the new test file under the `aletheia-v2-tests` job. 8 integration tests + 15 existing unit tests pass; `make lint` clean.
