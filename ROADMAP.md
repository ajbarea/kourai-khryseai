...
Last reviewed: 2026-04-26 (M1 fully shipped — Round 6 accept+discard validated end-to-end; `/clear` ANSI escape fix; M13 Forge Order Confirmation + M14 Metis-First parallel routing both shipped — the player-experience load-bearing pair is complete; M15 forge logging architecture planned; M3 tool-call streaming gap closed for Kallos+Dokimasia; M4 within-loop caching shipped; M10 speech-vs-action convention shipped; M11 GUI attachment send path shipped; /usage CLI command shipped + /reset_usage + Gemini pricing follow-on; Kokoro slow tests extracted + a2a-sdk pinned `<1.0` after protobuf-migration finding; agent-level READMEs for the 6 main pipeline agents; /project delete two-tier confirmation guard; **all 5 Round 6 M6-future bullets closed** — read_file directory rejection + branch slug whitelist (#34) + CLI greeting maiden-name attribution (#35) + WSL audio noise suppression (#36) + git context discovery for specialist agents in worktree (this PR); external-research sweep added 11 new M6 bullets on player-experience patterns from OSS-Claude-Code clones + Typer + the MCP 2025-11-25 / A2A 1.0 specs (#37) — see Shipped, revised M2/M7, and the new M6 "Surfaced from external research" subsection)
