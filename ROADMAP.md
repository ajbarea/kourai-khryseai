...
Last reviewed: 2026-05-02 (**M18 Phase 1 verified GREEN end-to-end** via the 2026-05-01 `make smoke-m18` run — full specialist cascade through hephaestus → metis → techne → dokimasia → kallos → mneme, `✨ Forged in 57.4s`, two commit groups, all defensive virtues code paths fired as designed. 2026-05-02 batch (thirteen PRs): `[#118]` dead AGENT_METADATA color fields, `[#119]` Okabe-Ito CVD-safe agent badges (closes #10), `[#120]` captions toggle for audio-only dialogue mode (closes #19), `[#121]` apt cache for portaudio19-dev, `[#122]` GHA layer cache for MCP Docker builds, `[#123]` NO_COLOR honored across the rest of the CLI palette, `[#124]` `/aj-deslop` sweep on recent justification prose (-73 LoC), `[#125]` migrated ten agents + vn_bridge + sandbox + 2 MCP sidecars from `python:3.12-slim` / `node:20-slim` → Chainguard Wolfi (closes #98 — 0H baseline vs 1H on Debian-glibc CVE-2026-5435; introduces `# research(2026-05):` inline-rationale convention), `[#127]` migrated `templates/backend/Dockerfile` so Techne-scaffolded player projects inherit the 0-CVE base, `[#128]` `/aj-deslop` sweep on `streaming.py` + `log.py` (-24 LoC), `[#129]` `/aj-deslop` sweep on CI workflow comment blocks (-21 LoC, closes the IMPL TODO), `[#130]` GH Actions scheduled rescan workflow for issue #126 (one-shot-ish, fires Saturdays from 2026-05-16 until upstream `@xmldom/xmldom` lands `>=0.8.13`), `[#131]` silence two pre-existing ty warnings surfaced by `/aj-ci-audit` (record.args tuple-narrowing in log.py + ContentKind literal in the captions test). Issue #126 filed for the upstream-blocked `@xmldom/xmldom@0.8.12` HIGH CVE bundled inside npm 11.13.0 — not exploitable in our threat model (xmldom only sees npm-internal config XML, never agent / network input). Same-day post-smoke DX cleanup: `[#114]` honest rebuild-failure timer, `[#115]` no-voice flag + `KOURAI_TTS` env honor + virtues fail-soft against readonly DB + smoke gate-ack regex narrowing, `[#116]` soft-fail banner also fires on pipeline abort before mneme, `[#117]` uvicorn.access filter silences Docker healthcheck log noise. M18 Phase 1 baseline (#99-#106): URI-namespaced extension key `https://kourai.khryseai/ext/streaming/v1` carries `{"content_kind": "dialogue" | "status" | "code" | "spec"}`, host CLI + vn_bridge both gate on it. M7 fully shipped 2026-04-30 (six phases, `Message.metadata` channel as the v1.0 boundary). M13 fix shipped 2026-04-30. M19 shipped 2026-04-29 (RealtimeTTS migration — "VHS rewind" failure mode gone by construction). M17 fully shipped 2026-04-29 (Phase 2 confidence decay closes the milestone). Sister-repo audit weekly cron (Monday 12:00 UTC, routine `trig_013uP9ryCLYscBKS7X6PB5og`). Active focus: M18 Phase 2 (SSML inside dialogue bodies — Kokoro doesn't natively consume SSML so transitional strip-then-synthesize, ElevenLabs migration on M6 unblocks full SSML downstream) and Phase 3 (`KIND_CODE` / `KIND_SPEC` distinct render paths + retire the `kind is None` legacy fallback now that the GREEN smoke confirmed no untagged production emissions). Pre-release perfection stance unchanged — May 2026 best practice no matter the cost; web-search before any implementation; architectural fix over expedient patch.)
...
- 2026-05-02 — **UX/DX + CI batch** shipped in seven PRs. `[#118]` dropped dead `color`/`hex_color` fields from `AGENT_METADATA` (three palettes silently disagreed; none were read) — net -19 LoC. `[#119]` per-agent Okabe-Ito CVD-safe badges in CLI comms-window headers (closes #10) — six specialists each get a distinct hue, dokimasia yellow uses bold-black for WCAG contrast, NO_COLOR + truecolor gates avoid the "hardcoded RGB bypasses terminal theme" failure mode (cf. Claude Code accessibility issue #34702). `[#120]` captions toggle for audio-only dialogue mode (closes #19) — `captions_enabled` setting at panel `[4]` gates KIND_DIALOGUE rendering; off+voice = audio-only, voice-off forces effective ON. `[#121]` apt-cache for `portaudio19-dev` via `awalsh128/cache-apt-pkgs-action@v1.6.0` — fixes 15+min Azure-mirror-flake stuck-on-apt failure mode (~5s on warm cache vs. 60-120s cold). `[#122]` GHA layer cache for MCP sidecar Docker builds via `docker/build-push-action@v7.1.0` + `docker/setup-buildx-action@v4.0.0` with separate `scope=` per image; ~1-2 min saved per integration run on warm cache. `[#123]` NO_COLOR honored across the rest of `styling.py` (gold separators, red error banners, cyan emphasis) — strip color, keep bold/italic/dim per spec. `[#124]` `/aj-deslop` sweep on the long justification prose I'd added in `[#117]` and `[#119]` — -73 LoC across 3 files, no behavior change; broadens the codebase comment-terseness rule from per-field to codebase-wide per AJ's pre-release stance. 2898 unit tests green throughout the batch; lint + format + ty clean.
- 2026-05-01 — **M18 Phase 1 GREEN end-to-end + post-smoke DX cleanup batch** shipped. First successful `make smoke-m18` run against M18-Phase-1-rebuilt containers: hephaestus → metis → techne → dokimasia → kallos → mneme, two commit groups generated, `✨ Forged in 57.4s`, full transcript at `logs/smoke-m18-20260501-091432Z.log`. Defensive virtues code (`[#115]`) fired exactly as designed — both dokimasia (arete) and kallos (techne_v) hit the readonly-DB write path, both logged the M5-cause warning, both continued; pipeline survived where it previously crashed. Four DX fixes followed. `[#114]` honest rebuild-failure timer: `make rebuild` no longer prints `[TIMER] Target rebuild completed in 0 seconds` when sub-steps fail in under half a second — stderr names the failing sub-step + exit code, timer line says `aborted at '<step>'` instead of `completed`, elapsed prints with sub-second precision. Same fix applied to every COMPOSITE_TASKS target (yolo, restart, dev, dev-vn). Bonus: leaf timed tasks now report `failed (exit N)` on non-zero exit. `[#115]` four bundled fixes uncovered by the live smoke: `--voice/--no-voice` Click negation (PortAudio crashed under WSL2 without it), `KOURAI_TTS` env honor in `_cli_command`, smoke gate-pattern regex narrowed to actual gate surfaces (`Light the forge?` / `Approve?` / `CONFIRM_ORDER` / `Forge cooled`) — previously matched routine `Analyzing`/`Forging` orchestrator status text, false-skipping the `yes` ack and timing out at 15min; virtues `update_virtue` / `_get_virtues_db` resilient to readonly database (M5 deferred-design issue acknowledged in Dockerfile but never patched in the sqlite layer — dokimasia's post-success `update_virtue` would crash with `sqlite3.OperationalError`, decorator wrapped as `InternalError`, hephaestus aborted mid-pipeline). `[#116]` soft-fail banner also fires on pipeline abort before mneme — original `[#109]` only caught `commit_count == 0`; new discriminator `commit_count is None` AND `final_state == TASK_STATE_FAILED` distinguishes "crash" (banner with distinct `aborted before Mneme` message) from "by-design non-mneme run" (COMPLETED, no banner). Also handles artifact-less abort path with `Forge aborted at N.Ns` red closing line. `[#117]` Docker healthcheck access-log noise silenced (closes IMPL `(#12)`): every agent's uvicorn served `GET /.well-known/agent-card.json` 200 OK INFO line every 15s, ~44 lines/min cluster-wide that drowned real events. New `_UvicornAccessPathFilter` in `kourai_common.log` installed on the `uvicorn.access` logger (logger-level, not handler — uvicorn configures its own StreamHandler outside the kourai root chain). WARNING+ records pass so 5xx healthchecks still surface. Live-verified: restarted aletheia → 0 lines in 30s vs 2 lines on idle peer. 2886 unit tests green; lint + format clean across all four PRs.
- 2026-04-30 — **Post-M18 UX/DX cleanup batch** shipped (six squash-merged PRs in one session, all flagged by the 2026-04-29 smoke). `[#107]` (#15) drops the empty trailing `Project root:` listing from metis's enriched prompt when the worktree iterdir returns no listable entries; `[#108]` (#22) silences phonemizer's per-call `words count mismatch` WARN spam via a surgical filter on the `phonemizer` logger that drops only the specific known-benign message and keeps other warnings audible (web-confirmed against bootphon/phonemizer#169 + Amphion/vall-e/Kokoro community threads); `[#109]` (#17) lifts mneme's `commit_count: 0` signal out of buried prose into an explicit red `⚠ No commits produced` banner above "Forged in" — added `extract_artifact_data` / `extract_parts_data` helpers in `kourai_common.a2a_events` for typed access to DataPart payloads; `[#110]` swaps literal `"python"` for `sys.executable` in three `TestRunCommand` tests so they pass on WSL2 hosts where only `python3` is on PATH; `[#111]` (#23) pre-warms one `KPipeline` per unique `lang_code` in `AGENT_VOICE_MAP` at `RealtimeTTSEngine.__init__` time so first Techne `bf_emma` speech no longer pays the lazy-pipeline-build pause (forward-compat — adding a new language voice auto-prewarms); `[#112]` (#14) updates `kourai_common.mcp_client.query_context7` to `@upstash/context7-mcp` v2.x's renamed tool (`query-docs` not `get-library-docs`) and reshaped params (`libraryId`+`query` not `context7CompatibleLibraryID`+`topic`+`tokens`), and adds a `_strip_transcript_markers` helper in `doc_lookup.py` so transcript role markers (`[User]:`, `[Hephaestus]:`) don't leak into Context7 library extraction. New tests lock the v2 tool name + param shape so a future server rename gets caught at the unit level. 2858 unit tests green; lint + format + ty clean across the batch
