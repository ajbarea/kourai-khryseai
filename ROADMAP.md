...
- 2026-05-25 — **Dependabot dependency automation** (`github-actions` only). `.github/dependabot.yml` on the canonical fleet shape (techne `templates/dependabot.yml.example`), auto-syncing the ~50 workflow action pins that were hand-maintained. `uv` deferred — the 18-member uv-workspace hits dependabot-core#14004 (open, mis-targets members as deps); docker skipped — `docker/*.Dockerfile` custom names aren't auto-detected (dependabot/feedback#145). Re-enable `uv` once #14004 closes.
- 2026-05-23 — **Aletheia v2 6-PR arc shipped** [#222 → #234]. Academic citation verifier as a second surface alongside v1 generic claim validation. Architecture: `kourai_common.citation_artifacts` (`PaperMetadata` / `TriangulationResult` / `ConflictReport` Pydantic dataclasses) + `kourai_common.triangulate` (rapidfuzz cross-source agreement) + `kourai_common.academic_search` (httpx + tenacity clients for S2 / arXiv / OpenAlex, cassette-replay tests). Agent surface in `agents/aletheia/agent.py`: 5-step ReAct loop (`extract_claim → search_papers → fetch_paper_text → match_evidence → triangulate → write artifact`) + `audit_existing_citations` scheduled drift check. **Anti-hallucination guarantees**: identity fields verbatim from API (never LLM-generated), triangulation gate (second-source agreement on DOI/arxiv_id/title/first-author/year), verbatim-excerpt substring check, artifact-file existence gate, re-verifiability via audit. Mechanical CI gate `scripts/check_citations.py` (no-LLM walker, flags `verified_at > 365d`) wired into pre-commit + dedicated workflow. Nightly `aletheia-v2-contracts` job calls each API once to surface upstream schema drift hours before the cassette-replay path breaks. User-facing docs at `docs/agents/aletheia.md`. **Critical CI gotcha encoded**: cassette files have a hard 5MB budget (vcrpy YAML overhead); anything bigger gets `@pytest.mark.nightly` instead of going into git. Optional `aletheia-v2` extra has its own `tests.yml` job so importorskip-gated tests actually run rather than silently skip. Phase 2 (proactive inline guard — Techne / Kallos call `verify_and_cite` when emitting citations) unblocked but un-prioritized pending real-usage data.
