"""Dialogue pacing controller for natural, light-novel-like conversation flow.

Manages the timing of agent responses to create a more natural reading/listening
experience instead of rapid-fire API responses.
"""
...
class DialoguePacer:
    ...
    def get_delay_between_responses(self) -> float:
        """Get delay between consecutive agent responses.
        ...
        if mode == PacingMode.INSTANT:
            return 0.0
        elif mode == PacingMode.FAST:
            return 0.5
        elif mode == PacingMode.NORMAL:
            return 1.5
        elif mode == PacingMode.SLOW:
            return 3.0
        else:  # CUSTOM
            return self.config.custom_delay

    def get_character_display_duration(self, text: str) -> float:
        """Calculate how long text should take to display/read.
    ...
    def set_mode(self, mode: PacingMode, custom_delay: float | None = None) -> None:
        ...
        Args:
            mode: New pacing mode.
            custom_delay: Custom delay if mode is CUSTOM.
        """
    ...
    def set_thinking_pause(self, enabled: bool, duration: float | None = None) -> None:
        """Configure thinking pause before responses.

        Args:
            enabled: Whether to show thinking pause.
